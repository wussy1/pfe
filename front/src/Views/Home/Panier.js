...
const Panier = ({ navigation }) => {
  ...
  async function makeCommand(){
    ...
    axios
      .post(
        `http://192.168.1.61:5000/api/comd/add`
      ,{
        "products": cart,
  ...
  async function getPanier() {
    await getUserData().then((res) =>
      axios
        .get(
          `http://192.168.1.61:5000/api/pan/by-user-id/${JSON.parse(res).id}`
        )
  ...
  async function PlusItem(idpanier, idproduit) {
    await axios
      .post("http://192.168.1.61:5000/api/pan/plus", {
        id_panier: idpanier,
        id_prod: idproduit,
      })
      .then(() => getPanier());
  ...
  async function MinusItem(idpanier, idproduit) {
    ...
    await axios
      .post("http://192.168.1.61:5000/api/pan/minus", {
        id_panier: idpanier,
        id_prod: idproduit,
      })
      .then(() => getPanier());
